...
if (uploadForm) {
  uploadForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (roomId) {
        socket.emit('report-shared', { roomId, filename: data.filename, url: data.path || `/api/video_uploads/${data.filename}` });
      }

      // Show in local list
      addReportItem({ filename: data.filename, url: data.path || `/api/video_uploads/${data.filename}` });
    } catch (err) {
